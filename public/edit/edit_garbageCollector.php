...
if (isset($_GET['id']) && ctype_digit($_GET['id'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM garbage_collectors WHERE id = ?");
    ...
    $joiningDate = $collector['joining_date'];
...
if (isset($_POST['edit_collector'])) {
    ...
    if ($salary === '' || !is_numeric($salary)) $errors['salary'] = 'Please enter valid salary';

    /* ===== DUPLICATE MOBILE CHECK ===== */
    if (empty($errors)) {
        $check = $pdo->prepare("
            SELECT id FROM garbage_collectors 
            WHERE mobile = ? AND id != ?
        ");
...
<div class="container-fluid px-4 mb-4">
    ...
    <ol class="breadcrumb mb-4">
        ...
        <li class="breadcrumb-item active">Edit Garbage Collector</li>
    </ol>
